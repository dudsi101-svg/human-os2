...
export function Sparkline({ points, unit, label }: {
  ...
  // Alternatywa tekstowa wykresu: zakres wartości, liczba punktów, ostatnia
  // wartość — czytnik ekranu dostaje sedno trendu bez oglądania linii.
  const chartDescription =
    `${label ?? "Wykres"}: ${points.length} pomiarów od ${points[0].x} ` +
    `do ${points[points.length - 1].x}, zakres ${min.toFixed(1)}–${max.toFixed(1)} ${unit}, ` +
    `ostatnia wartość ${points[points.length - 1].y} ${unit}`;
  return (
    <div>
