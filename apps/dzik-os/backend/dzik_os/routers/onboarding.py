...
reguły adaptacji i lista objawów alarmowych żyją w `onboarding_flow` i
działają identycznie z modelem językowym i bez niego. Model (jeśli
operator skonfigurował dostawcę, a klient wyraził zgodę `funkcje_ai`)
może wyłącznie przygotować WERSJĘ ROBOCZĄ podsumowania — nie prowadzi
rozmowy, nie decyduje, nie publikuje planu ani diety.

Dwie odrębne akceptacje (Konstytucja Human OS, granice roli AI):

1. **klient** zatwierdza podsumowanie swoich danych — dopiero wtedy
   trafiają one do profilu normalną, wersjonowaną ścieżką
   (`profile_service.apply_profile_fields`) i do celów (`Goal`);
...
def _require_client_self(user: User, client_id: str) -> None:
    ...
    if user.id != client_id:
        deny(user.id, f"onboarding:{client_id}")


def _require_coach(db: Session, user: User, client_id: str) -> None:
...
def _latest_session(db: Session, client_id: str) -> OnboardingSession | None:
    return (
        db.query(OnboardingSession)
        .filter(OnboardingSession.client_id == client_id)
        .order_by(OnboardingSession.started_at.desc())
        .first()
    )
...
def _state_payload(db: Session, user: User, session: OnboardingSession | None, client_id: str):
    if session is None:
        return {
            "session": None,
            "step": None,
    ...
    planned = plan_steps(values, allowed_domains=domains)
    visible = _visible_sensitive(db, user, client_id)
    step = STEP_BY_ID.get(session.current_step_id or "")
    current_answer = next(
    ...
    return {
        ...
        "answers": [
            _answer_out(r, visible) for r in _answers(db, session.id, current_only=False)
        ],
...
def _ai_status(db: Session, client_id: str) -> dict:
    ...
    Brak zgody i brak dostawcy to dwie różne informacje: pierwsza jest
    decyzją klienta, druga konfiguracją operatora."""
...
def start(
    ...
    _require_client_self(user, client_id)
    session = _open_session(db, client_id)
    created = False
    if session is None:
        session = OnboardingSession(id=new_id("ONB"), client_id=client_id)
        db.add(session)
        db.flush()
        created = True
    rows = _answers(db, session.id)
    values = _answer_values(rows)
    planned = plan_steps(values, allowed_domains=allowed_domains(db, client_id))
    ...
    if created:
        record_event(
            ...
            summary="Rozpoczęto rozmowę startową (onboarding)",
        )
    db.commit()
    return _state_payload(db, user, session, client_id)
...
def answer(
    ...
    (także sprzeczna) zostaje. Odpowiedź z objawem alarmowym dostaje
    spokojny komunikat kierujący do pomocy medycznej i NIE jest wysyłana
    do dostawcy modelu."""
    resolve_client_access(db, user, client_id, action="write", domain=DOMAIN_COLLABORATION)
    _require_client_self(user, client_id)
    session = _open_session(db, client_id)
    ...
    step = STEP_BY_ID.get(body.step_id)
    if step is None:
    ...
    db.add(
        OnboardingAnswer(
            ...
            safety_signals=json.dumps(signals, ensure_ascii=False) if signals else None,
            version=version,
        )
    )
    db.flush()
    if signals:
        ...
        record_event(
            db,
            action="ONBOARDING_SAFETY_FLAGGED",
            actor_id=user.id,
            subject_ids=[client_id],
            payload={"session_id": session.id, "step_id": step.id, "signals": signals},
            summary="Rozmowa startowa: odpowiedź skierowana do konsultacji medycznej",
        )
    rows = _answers(db, session.id)
    values = _answer_values(rows)
    planned = plan_steps(values, allowed_domains=domains)
    ...
    payload["safety_notice"] = (
        {"message": SAFETY_MESSAGE, "signals": signals} if signals else None
    )
    return payload
...
def go_back(
    ...
    if session is None:
        raise HTTPException(status_code=404, detail="Rozmowa nie została rozpoczęta")
    rows = _answers(db, session.id)
    planned = plan_steps(_answer_values(rows), allowed_domains=allowed_domains(db, client_id))
...
def _store_summary(db: Session, session: OnboardingSession, items: list[dict]) -> None:
...
def build_summary(
    ...
    Zawsze powstaje wersja deterministyczna. Jeśli klient wyraził zgodę
    `funkcje_ai`, dostawca jest skonfigurowany i limity nie są wyczerpane —
    model dostaje ZMINIMALIZOWANY zestaw odpowiedzi i może zaproponować
    zwięźlejsze wartości wraz z poziomem pewności. Odpowiedź niezgodna ze
    schematem jest odrzucana (jedno ponowienie), a podsumowanie zostaje
    deterministyczne."""
    resolve_client_access(db, user, client_id, action="write", domain=DOMAIN_COLLABORATION)
    _require_client_self(user, client_id)
    session = _open_session(db, client_id)
    ...
    if not ai_consent_active(db, client_id):
        ...
        result = onboarding_ai.request_summary_draft(db, user_id=client_id, answers=for_ai)
        if result.ok:
            mode = "AI_DRAFT"
    ...
    record_event(
        db,
        action="ONBOARDING_SUMMARY_BUILT",
        actor_id=user.id,
        subject_ids=[client_id],
        payload={"session_id": session.id, "mode": mode, "fields": len(items)},
        summary=f"Przygotowano podsumowanie rozmowy startowej (tryb {mode})",
    )
    db.commit()
    return _state_payload(db, user, session, client_id)
...
def client_approve(
    ...
    profilu (append-only, z proweniencją CLIENT_DECLARED) i do celów.
    ...
    goal_id = _ensure_main_goal(db, client_id, goal_title, goal_target)
    ...
    record_event(
        db,
        action="ONBOARDING_CLIENT_APPROVED",
        actor_id=user.id,
        subject_ids=[client_id],
        payload={
            "session_id": session.id,
        ...
        summary="Klient zatwierdził podsumowanie rozmowy startowej",
    )
    db.commit()
    payload = _state_payload(db, user, session, client_id)
...
def _ensure_main_goal(
    ...
    goal = Goal(
        ...
        client_id=client_id,
        title=title[:300],
        description=f"Termin wskazany w rozmowie startowej: {target}" if target else None,
        kind="MAIN",
...
def coach_review(
    ...
    _require_coach(db, user, client_id)
    resolve_client_access(db, user, client_id, domain=DOMAIN_COLLABORATION)
    session = _open_session(db, client_id) or _latest_session(db, client_id)
    payload = _state_payload(db, user, session, client_id)
    ...
    payload["can_approve"] = bool(
        session is not None and session.status in ("CLIENT_APPROVED", "COACH_APPROVED")
    )
    return payload
...
def coach_approve(
    ...
    pending = [i.field_key for i in items if i.needs_confirmation and not i.coach_confirmed]
    if pending:
        raise HTTPException(
            status_code=409,
    ...
    record_event(
        db,
        action="ONBOARDING_COACH_APPROVED",
        actor_id=user.id,
        subject_ids=[client_id],
        payload={
            "session_id": session.id,
        ...
        summary="Trener zatwierdził podsumowanie rozmowy startowej",
    )
    db.commit()
    return _state_payload(db, user, session, client_id)
