...
| GET /api/clients/{id}/cardio/prefill; POST …/cardio/podglad (0.73.0) | T | propozycja cardio z suwaków dla własnego klienta — propose-only, nic nie zapisuje; bramka zdrowotna przed silnikiem (blok zdrowotny nie trafia do planu/śladu/audytu); wiek, tętno spoczynkowe i masa czytane z danych klienta **tylko przy dostępie do domeny zdrowotnej** (`health_access`), inaczej tryb RPE; klient = 403, trener bez relacji = 404. **Pochodne:** wynik wstawiony do planu (domena treningowa) niesie `hrmax_estimate` i zakres ud./min (pozwalają odtworzyć HRmax/RHR) — świadoma akceptacja jak przy każdej treści planu; odpowiedź o lekach nie jest zapisywana; `avg_hr` w dzienniku maskowane bez dostępu zdrowotnego i ignorowane przy zapisie przez trenera bez tego dostępu | tak | tak | R |
...
| GET /api/clients/{id}/profile/hints?area= | W, T | podpowiedzi z rozmów przy obszarze pracy (PLAN/DIETA/HARMONOGRAM/WSPOLPRACA): dosłowne deklaracje z pól profilu, filtr zgód identyczny jak profil | T: tak | T: tak (collaboration) | R |
