...
| GET /api/monitoring/summary, /records, /training | W (self), T (`client_id`) | postępy jednego klienta; 404 bez flagi `monitoring_tab_enabled`; trener bez `client_id` → 422 | T: tak | T: tak (dane treningowe) | R |
| GET /api/monitoring/body | W (self; klient z flagą zdrowotną → 404), T (`client_id`) | sylwetka: waga (średnia), obwody, zdjęcia | T: tak | T: tak (dane zdrowotne; zdjęcia osobna domena) | R |
