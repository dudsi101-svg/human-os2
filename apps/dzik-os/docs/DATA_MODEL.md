...
| `documents` | metadane dokumentu klienta (tytuł, kategoria) → files; migracja nr 20 (NULLable): `ocr_text` — tekst przepisany ze skanu i ZATWIERDZONY przez człowieka (dzięki niemu dokument da się wyszukać), `ocr_engine`, `ocr_at`. Oryginał pliku pozostaje bez zmian. |
| `ocr_tasks` | zadania przepisywania tekstu ze zdjęcia (migracja nr 20): owner_user_id (podmiot danych), created_by, file_id, purpose (PRODUKT/PLAN/DOKUMENT), document_id, status (PENDING/RUNNING/DONE/FAILED), engine (LOCAL/EXTENDED), mode_reason, text, proposal_json, error_code/error, chars, duration_ms, approved_at, result_ref, czasy. **Wynik to propozycja** — dopóki człowiek nie zatwierdzi, nie powstaje ani produkt, ani tekst przy dokumencie. Tekst i propozycja to dane osobowe (bywa: zdrowotne): wchodzą do eksportu, znikają przy usunięciu konta, nigdy nie trafiają do logów. Szczegóły: `OCR.md`. |
...
| `food_products` | baza produktów: name, category, kcal_100g/protein_100g/fat_100g/carbs_100g, default_portion_g, status oraz (migracja nr 18, wszystkie NULLable) fiber_100g, unit_name/unit_grams (jednostka sztukowa), source, note; (migracja nr 20, NULLable) origin_kind/origin_file_id/origin_engine — proweniencja wpisu utworzonego ze zdjęcia etykiety. **Broadcast**, ten sam wzorzec co `knowledge_items`. Przeliczenie na porcję (kcal/makro × gramatura/100, także z liczby sztuk) jest czystą arytmetyką — po stronie frontendu (`foodUtils.ts`) i w bezstanowym `POST /api/food-products/portion`; nie jest osobnym magazynem danych. Katalog, format CSV i plan wycofania migracji: `BAZA_PRODUKTOW.md`. |
